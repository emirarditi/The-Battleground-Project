...
public class BattlePhase extends JPanel {
	...
	public BattlePhase() { // Constructor
		Main.isCounter = false; // Makes Counter() to stop.
		STOP_THREADS = false; // Makes BattlePhase() threads' run.
		myHero = Main.characters[0]; // YOU.
		Opponent = Main.characters[1]; // OPPONENT.
		myHero_MAXHEALTH = myHero.getHealth();
		Opponent.setHealth((int) (Opponent.getHealth() * 1.35)); // +35% HP
		...
		button1.setToolTipText("Name: " + myHero.getQName() + ", Damage: "
				+ myHero.getQDamage() + ", Miss Chance: 13%"); // DESCRIPTION
		button2.setToolTipText("Name: " + myHero.getWName() + ", Damage: "
				+ myHero.getWDamage() + ", Miss Chance: 17%"); // DESCRIPTION
		button3.setToolTipText("Name: " + myHero.getEName() + ", Damage: "
				+ myHero.getEDamage() + ", Miss Chance: 29%"); // DESCRIPTION
		if (myHero instanceof ArtificialIntelligence) // for Heal Spell
			button4.setToolTipText("Name: " + myHero.getRName() + ", HP: +"
					+ myHero.getRDamage() + ", Miss Chance: 40%"); // DESCRIPTION
		else
			button4.setToolTipText("Name: " + myHero.getRName() + ", Damage: "
					+ myHero.getRDamage() + ", Miss Chance: 20%"); // DESCRIPTION
		add(button1);
		add(button2);
		add(button3);
		add(button4);
		...
		label1.setToolTipText("Name: " + Opponent.getQName() + ", Damage: "
				+ Opponent.getQDamage() + ", Miss Chance: 13%"); // DESCRIPTION
		label2.setToolTipText("Name: " + Opponent.getWName() + ", Damage: "
				+ Opponent.getWDamage() + ", Miss Chance: 17%"); // DESCRIPTION
		label3.setToolTipText("Name: " + Opponent.getEName() + ", Damage: "
				+ Opponent.getEDamage() + ", Miss Chance: 29%"); // DESCRIPTION
		if (Opponent instanceof ArtificialIntelligence) // for Heal Spell
			label4.setToolTipText("Name: " + Opponent.getRName() + ", HP: +"
					+ Opponent.getRDamage() + ", Miss Chance: 40%"); // DESCRIPTION
		else
			label4.setToolTipText("Name: " + Opponent.getRName() + ", Damage: "
					+ Opponent.getRDamage() + ", Miss Chance: 20%"); // DESCRIPTION
		add(label1);
		add(label2);
		add(label3);
		add(label4);
		// ========================================================== COOLDOWNS
		Cooldown1 = new JLabel(new ImageIcon("FALSE.png"));
		Cooldown1.setBounds(119, 591, 15, 15);
		Cooldown2 = new JLabel(new ImageIcon("FALSE.png"));
		Cooldown2.setBounds(139, 591, 15, 15);
		Cooldown3 = new JLabel(new ImageIcon("FALSE.png"));
		Cooldown3.setBounds(159, 591, 15, 15);
		add(Cooldown1);
		add(Cooldown2);
		add(Cooldown3);
		// ============================================================ ADD-ONS
		mid = new JLabel(new ImageIcon("Skill Circle.png"));
		mid.setBounds(350, 175, 300, 300);
		shield1 = new JLabel(new ImageIcon("Empty.png"));
		shield1.setBounds(250, 85, 50, 50);
		shield2 = new JLabel(new ImageIcon("Empty.png"));
		shield2.setBounds(700, 85, 50, 50);
		critical1 = new JLabel(new ImageIcon("Empty.png"));
		critical1.setBounds(20, 85, 50, 50);
		critical2 = new JLabel(new ImageIcon("Empty.png"));
		critical2.setBounds(930, 85, 50, 50);
		crithit = new JLabel(new ImageIcon("Empty.png"));
		crithit.setBounds(715, 149, 250, 153);
		critheal = new JLabel(new ImageIcon("Empty.png"));
		critheal.setBounds(715, 149, 150, 150);
		Font font = new Font("Harrington", 8, 30); // New Font
	...
	class MyHealth extends Thread {
		...
		MyHealth() {
			...
			label = new JLabel(myHero.getChracterName() + "'s HP: "
					+ String.valueOf(myHero.getHealth()) + "/"
					+ myHero_MAXHEALTH + "  (YOU)");
		...
		public void run() {
			while (STOP_THREADS == false) { // To Stop Threads
				...
				while (myHero.getHealth() < life && life > 0) {
					...
					label.setText(myHero.getChracterName() + "'s HP: "
							+ String.valueOf(life) + "/" + myHero_MAXHEALTH
							+ "  (YOU)");
				...
				while (myHero.getHealth() > life && life > 0
						&& life < myHero_MAXHEALTH) { // For Heal Skill
					life++;
					healthlabel.setText(String.valueOf(life));
					label.setText(myHero.getChracterName() + "'s HP: "
							+ String.valueOf(life) + "/" + myHero_MAXHEALTH
	...
	class OpponentHealth extends Thread {
		...
		OpponentHealth() {
			...
			label = new JLabel(Opponent.getChracterName() + "'s HP: "
					+ String.valueOf(Opponent.getHealth()) + "/"
					+ Opponent_MAXHEALTH + "  (OPPONENT)");
		...
		public void run() {
			while (STOP_THREADS == false) { // To Stop Threads
				...
				while (Opponent.getHealth() < life && life > 0) {
					life--;
					healthlabel.setText(String.valueOf(life));
					label.setText(Opponent.getChracterName() + "'s HP: "
							+ String.valueOf(life) + "/" + Opponent_MAXHEALTH
							+ "  (OPPONENT)");
					progress.setValue((100 * life) / initialHealth);
					try {
						sleep(20);
				...
				while (Opponent.getHealth() > life && life > 0
						&& life < Opponent_MAXHEALTH) { // For Heal Skill
					life++;
					healthlabel.setText(String.valueOf(life));
					label.setText(Opponent.getChracterName() + "'s HP: "
							+ String.valueOf(life) + "/" + Opponent_MAXHEALTH
	...
	public class AttackModifier extends MouseAdapter {
	...
	public class Battle extends Thread {
		...
		public void run() {
			while (myHero.getHealth() > 0 && Opponent.getHealth() > 0
					&& STOP_THREADS == false) {
				isOpponent = false;
				mid.setIcon(new ImageIcon("Skill Circle.png"));
				// It is your turn...
				currentAttack.setText("It is your turn...");
				while (!isOpponent) {
					System.out.println();// Player's thinking time (Infinite)
				}
				...
				if (Opponent.getHealth() > 0) {
					...
					critheal.setIcon(new ImageIcon("Empty.png"));
					crithit.setIcon(new ImageIcon("Empty.png"));
					mid.setIcon(new ImageIcon("Skill Circle.png"));
					...
					JLabel youwin = new JLabel(new ImageIcon("Won.png"));
					youwin.setBounds(250, 315, 50, 50);
					youwin.setToolTipText("Winner Award.");
					add(youwin); // Trophy.
					currentAttack
							.setText("<html>Opponent lost the match, <br/>You WIN!!");
					// WE WIN!! YES!!
			...
			if (myHero.getHealth() <= 0) { // OPPONENT WINS, UNFORTUNATE ALWAYS
				// FINDS THE WAY TO REACH ME :(
				try {
					sleep(500); // 2000 + 500 = 2500
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
				JLabel opwins = new JLabel(new ImageIcon("Won.png"));
				opwins.setBounds(700, 315, 50, 50);
				opwins.setToolTipText("Winner Award.");
				add(opwins); // Trophy.
				currentAttack
						.setText("<html>You lost the match, <br/>OPPONENT WINS!!");
	...
	public void cooldownGenerator() { // Paints the cooldown boxes
		if (myHero.getCountyCounter() == 1)
			Cooldown1.setIcon(new ImageIcon("TRUE.png"));
		if (myHero.getCountyCounter() == 2)
			Cooldown2.setIcon(new ImageIcon("TRUE.png"));
		if (myHero.getCountyCounter() == 3)
			Cooldown3.setIcon(new ImageIcon("TRUE.png"));
		if (myHero.getCountyCounter() == 0) {
			Cooldown1.setIcon(new ImageIcon("FALSE.png"));
			Cooldown2.setIcon(new ImageIcon("FALSE.png"));
			Cooldown3.setIcon(new ImageIcon("FALSE.png"));
		}
	}

	public void shieldPainter() { // Paints shields
		if (myHero.hasHeroMissed == true) {
			shield2.setIcon(new ImageIcon("Shield.png"));
			shield2.setToolTipText("Opponent dodged/blocked your last spell.");
		}
		if (myHero.hasHeroMissed == false) {
			shield2.setIcon(new ImageIcon("Empty.png"));
			shield2.setToolTipText("");
		}
		if (Opponent.hasOpponentMissed == true) {
			shield1.setIcon(new ImageIcon("Shield.png"));
			shield1.setToolTipText("You dodged/blocked opponent's last spell.");
		}
		if (Opponent.hasOpponentMissed == false) {
			shield1.setIcon(new ImageIcon("Empty.png"));
			shield1.setToolTipText("");
		}
	}

	public class hitAnimation extends Thread {
		...
		public void run() {
			if (STOP_THREADS == false) { // To Stop Threads
				shieldPainter();
				if (Attacker.hasHeroMissed == false
						&& Attacker.hasOpponentMissed == false) {
					if (Attacker instanceof ArtificialIntelligence
							&& Attacker.RUsed == true) {
						for (int j = 0; j < 15; j++) {
							dy -= (j / 2);
	...
	public class Victory extends Thread {
		...
		public void run() {
			if (STOP_THREADS == false) { // To Stop Threads
				...
				mid.setIcon(new ImageIcon("RIP.png")); // RIP !!
	...
	class NewGame implements ActionListener {
		public void actionPerformed(ActionEvent arg0) {
			...
			// It saves the CPU of the computer by stopping them.
			// Otherwise, CPU usage can be seen about %90 after
			// 3 or more new games.
			try {
				Main main = new Main();
			} catch (Exception e) {
