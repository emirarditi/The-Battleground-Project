...
public abstract class Character {
	...
	// All characters are equal powered because their health powers and damages
	// is adjusted by a MATH formula..
	// If x is constant, U is damage and HP is health power,
	// the formula is x = U x HP
	// Q constant is 2100, W constant is 2800, E constant is 4620, R constant is
	// 7000.. The character with low HP means has stronger spells.
	// The character with high HP means has weaker spells.

	public Character(String CharacterName, String QName, String WName,
			String EName, String RName, int Health, int QDamage, int WDamage,
			int EDamage, int RDamage) { // Constructor
	...
	public void Q() { // Q (Weakest) Skill
		Random random = new Random();
		int r = random.nextInt(100);
		if (r <= 12) { // 13% MISS CHANCE
			BattlePhase.currentAttack
					.setText("<html>You missed...<br/>Opponent dodged your: <br/>"
							+ BattlePhase.myHero.getQName());
			...
			BattlePhase.Opponent.setHealth(BattlePhase.Opponent.getHealth()
					- (getQDamage() * 2));
			BattlePhase.currentAttack.setText("<html>You used: <br/>"
					+ BattlePhase.myHero.getQName()
					+ "<br/>!! CRITICAL HIT !!<hmtl>");
			BattlePhase.mid.setIcon(new ImageIcon(getQMidIcon())); // Middle
			// Icons
			hasHeroMissed = false;
			CountyCounter++;
	...
	public void W() { // W (Average) Skill
		...
		if (r <= 16) { // 17% MISS CHANCE
			...
			BattlePhase.currentAttack.setText("<html>You used: <br/>"
					+ BattlePhase.myHero.getWName()
					+ "<br/>!! CRITICAL HIT !!<hmtl>");
			BattlePhase.mid.setIcon(new ImageIcon(getWMidIcon())); // Middle
			// Icons
			hasHeroMissed = false;
			CountyCounter++;
			BattlePhase.critical1.setIcon(new ImageIcon("Critical.png"));
	...
	public void E() { // E (Strongest Of Basics) Skill
		Random random = new Random();
		int r = random.nextInt(100);
		if (r <= 28) { // 29% MISS CHANCE
			BattlePhase.currentAttack
					.setText("<html>You missed...<br/>Opponent dodged your: <br/>"
							+ BattlePhase.myHero.getEName());
			BattlePhase.mid.setIcon(new ImageIcon("Missed.png")); // Middle
			// Icons
			hasHeroMissed = true;
			BattlePhase.critical1.setIcon(new ImageIcon("Empty.png"));
			...
			BattlePhase.currentAttack.setText("<html>You used: <br/>"
					+ BattlePhase.myHero.getEName()
					+ "<br/>!! CRITICAL HIT !!<hmtl>");
			BattlePhase.mid.setIcon(new ImageIcon(getEMidIcon())); // Middle
			// Icons
			hasHeroMissed = false;
			CountyCounter++;
			BattlePhase.critical1.setIcon(new ImageIcon("Critical.png"));
